...
module.exports = async (uri, params, req, res) => {
    ...
    if (operation === 'DisruptionsAt') {
        if (pathSplat.length >= 2) {
            const codes = pathSplat[1];
            const data = await jsonRequest(`StopPoint/${codes}/Disruption`);
            return { type: 'application/json', data };
        }
        else {
            return { err: new ServerError(400, 'No station codes specified for operation DisruptionsAt.') };
        }
    }

    if (operation === 'LineDisruptions') {
        if (pathSplat.length >= 2) {
            const codes = pathSplat[1];
            const data = await jsonRequest(`Line/${codes}/Disruption`);
            return { type: 'application/json', data };
        }
        else {
            return { err: new ServerError(400, 'No line codes specified for operation LineDisruptions.') };
        }
    }
